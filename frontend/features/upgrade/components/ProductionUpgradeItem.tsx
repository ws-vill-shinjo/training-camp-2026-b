...
export function ProductionUpgradeItem({ id }: Props) {
  ...
  const encyclopedia = registry.encyclopedia[id];
  ...
  const displayYield = effectiveStat
    ? Number(effectiveStat.yield)
    : calcYield(config, displayLevel);
  const displayCycleSeconds = effectiveStat
    ? effectiveStat.cycleMs / 1000
